					...
					struct v4l2_format *f)
{
...
#if 0
        printk(KERN_ERR "YYYYY ctrl=%x width=%d,height=%d s3c_fimc_v4l2_s_fmt_vid_cap\n",ctrl,f->fmt.pix.width,f->fmt.pix.height);
#endif
 ...
 #if 0
        printk(KERN_ERR "XXX ctrl=%x width=%d,height=%d s3c_fimc_v4l2_s_fmt_vid_cap\n",ctrl,ctrl->out_frame.width,ctrl->out_frame.height);
#endif
	return 0;
}
...
#if 0
        err("XXXXXX in  s3c_fimc_v4l2_streamon %d,%d \n",ctrl->out_frame.width,ctrl->out_frame.height);
#endif
