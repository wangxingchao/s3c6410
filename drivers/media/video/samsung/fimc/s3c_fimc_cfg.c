...
int s3c_fimc_alloc_output_memory(struct s3c_fimc_out_frame *info)
{
	int ret;
 #if 0
        printk(KERN_ERR "XXXXXX  %d,%d  s3c_fimc_alloc_output_memory\n",info->width,info->height);
#endif
					...
					struct v4l2_pix_format *fmt)
{
	...
	int depth = 0;
#if 0
        printk(KERN_ERR "AAAA %d,%d s3c_fimc_set_output_format\n",fmt->width,fmt->height);
#endif
	frame->width = fmt->width;
	frame->height = fmt->height;
...
#if 0
        printk(KERN_ERR "XXXXX SSSS width=%d,height=%d s3c_fimc_set_output_frame \n",frame->width,frame->height);

        printk(KERN_ERR "XXXXX MMMM width=%d,height=%d s3c_fimc_set_output_frame \n",fmt->width,fmt->height);
#endif
...
#if 0
        printk(KERN_ERR "XXXXX EEE  width=%d,height=%d s3c_fimc_set_output_frame \n",ctrl->out_frame.width, ctrl->out_frame.height);
#endif
...
#if 0
        err("XXXX  in s3c_fimc_set_scaler_info,%d,%d\n",tx,ty);
#endif
...
#if 0
        err(" XXXXXXXX in s3c_fimc_start_dma %d,%d\n",ctrl->out_frame.width,ctrl->out_frame.height);
#endif
